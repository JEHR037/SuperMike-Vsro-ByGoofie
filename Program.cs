...
namespace ExploitFilter
{
    class Program
    {
        ...
        static void ConsolePoolThread()
        {
            while (true)
            {
                ...
                if (cmd == "/reload")
                {
                    ...
                    // Load AllowSkillsInPvP
                    Program.AllowSkillsInPvP();
        ...
        #region PvP Allow Skills
        public static void AllowSkillsInPvP()
        {

            // Check if file exist
            if (!File.Exists("skills/WhiteskillsInPvP.txt"))
            {
                // Inform user
                Console.ForegroundColor = ConsoleColor.DarkYellow;
                Console.WriteLine(FilterMain.FILTER + "skills/WhiteskillsInPvP.txt doesn't exist!");
                Console.ResetColor();
            }
            else
            {
                foreach (string line in File.ReadAllLines("skills/WhiteskillsInPvP.txt"))
                {
                    if (line != String.Empty && line != null)
                    {
                        // Add blocked skill
                        FilterMain.AllowSkillsInPvP.Add(Convert.ToInt32(line));
                    }
                }

                // Inform?
                Console.ForegroundColor = ConsoleColor.DarkCyan;
                Console.WriteLine(FilterMain.FILTER + "skills/WhiteskillsInPvP.txt was loaded!");
                Console.ResetColor();
            }
        }
        ...
        {
            // FILTER NAME
            Console.Title = FilterMain.FILTER + "- by Madelyn & Layered (" + FilterMain.UPDATED + ")";
