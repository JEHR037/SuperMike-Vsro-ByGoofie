...
namespace ExploitFilter.NetEngine
{
    sealed class AgentContext2
    {
        ...
        */
        ...
        // Client list game
        public static List<AgentContext2> clientlistgame = new List<AgentContext2>();
        ...
        public AgentContext2(Socket ClientSocket, AsyncServer.delClientDisconnect delDisconnect)
        {
            this.m_delDisconnect = delDisconnect;
            this.m_ClientSocket = ClientSocket;
            this.m_HandlerType = AsyncServer.E_ServerType.AgentServer2;
            ...
            try
            {
                if (FilterMain.PIONEER)
                {
                    if (this.connect)
                    {
                        this.m_ModuleSocket.Connect(new IPEndPoint(IPAddress.Parse(FilterMain.REMOTE_AGENT_2), FilterMain.AGENT2_LISTEN_PORT));
                    }
                }
                else
                {
                    if (this.connect)
                    {
                        this.m_ModuleSocket.Connect(new IPEndPoint(IPAddress.Parse(FilterMain.AGENT_IP2), FilterMain.AGENT2_LISTEN_PORT));
                    }
                }
                        ...
                        try
                        {
                            ...
                            if (nBps > FilterMain.dMaxBytesPerSec_Agent)
                            {
                                ...
                                if (FilterMain.ERROR_LOG.Equals("unknown") || FilterMain.ERROR_LOG.Equals("all") || FilterMain.ERROR_LOG.Equals("exploit"))
                                {
                                    ...
                                    Console.WriteLine(FilterMain.DATE + "[" + module_name + "] Client disconnected due to high BPS: {0} / {1}", this.ip, nBps);
                                    Console.ResetColor();
                                }
        ...
        public static void SendNoticeOnline(string Message)
        {
            try
            {
                foreach (AgentContext2 current in clientlistgame)
                {
            ...
            try
            {
                foreach (AgentContext2 current in clientlistgame)
                {
                    if (current.m_ClientSocket.Connected)
                    {
                        ...
                        {
                            // Send a notice
                            current.SendNotice("You have been disconnected from the server!");
                ...
                foreach (AgentContext2 current in clientlistgame)
                {
                    if (current.m_ClientSocket.Connected)
                    {
                        if (current.user_id.ToLower().Equals(search) || current.charname.ToLower().Equals(search))
                        {
                            ...
                            // Send a notice
                            current.SendNotice("You have been banned from the server!");
